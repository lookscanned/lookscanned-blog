...
While Hugo PaperMod theme includes a built-in language selection feature, it simply displays all available languages in the header. For blogs supporting numerous languages (like Look Scanned, which supports over 20), this approach clutters the header with language options. A custom dropdown-based language selector provides a more elegant solution.

![Default Language Selector in Hugo PaperMod theme](./old-language-select.webp)

## Implementation Guide

### Locate and Override the Original Layout

The original language selector is defined in `themes/PaperMod/layouts/partials/header.html`. To customize it, we need to create our own version at `layouts/partials/header.html`. Here's the original implementation:
...
### Create the Custom Dropdown Selector

Our custom implementation uses a language icon from ionicons5 as a trigger button, which reveals a dropdown menu containing all available languages. Here's the implementation:
...
To make the dropdown work smoothly and look consistent with the theme, add the following CSS:
...
The result is a clean, dropdown-based language selector that appears when hovering over the language icon:

![Custom Language Selector in Hugo PaperMod theme](./custom-language-select.webp)

You can find the complete implementation in this [GitHub commit](https://github.com/lookscanned/lookscanned-blog/commit/a47f5c2be887ab3ae198d1967f328d3683504ff0).
