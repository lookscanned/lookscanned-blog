...
즉 화면에서는 가려져 보여도, 실제로는 아래 텍스트나 OCR, 잔여 객체가 그대로 남아 전송될 수 있다. 이게 바로 **비효율적 레닥션**이다. 고급 해킹 문제가 아니라, “덮기”와 “삭제”를 혼동한 워크플로 문제다.
...
나는 **최종 내보내기 파일**만 확인한다(실제로 보낼 파일).

- 민감 키워드 **검색**(이름, ID, 이메일 조각, 주소)
- 검은 영역 주변을 선택해서 **복사/붙여넣기** 후 일반 텍스트 편집기에서 확인
- **서로 다른 보기 환경 2개**에서 열어보기(데스크톱 앱 + 브라우저면 보통 충분)
- 남아 있는 **주석/댓글** 확인(하이라이트, 노트, 도형)
- 외부 전달 시 **메타데이터**(작성자/제목/주제) 점검

문서가 스캔본에서 시작했거나 OCR을 거쳤다면 더 조심한다. 숨겨져 있지만 검색되는 텍스트는 놓치기 쉬운 숨은 레이어이기 때문이다.
...
1) **진짜 레닥션 수행**(겹쳐 가리는 게 아니라 콘텐츠 삭제)  
2) **부가 레이어 정리**(주석, 첨부, 숨김 레이어, 메타데이터)  
3) **최종 내보내기 검증**(위 점검 목록 기준)  
4) **전송용 버전 생성**(대개 스캔본 느낌으로, 일관된 최종 형태)

마지막 단계는 생각보다 중요하다. 보여주기식 보안을 위한 게 아니라, 기기별 우발적 깨짐을 줄이고 결과를 더 일관되게 만들기 위해서다.

## 내 프로세스에서 Look Scanned의 위치

나는 Look Scanned를 레닥션 도구로 쓰지 않는다. 그 용도가 아니다.  
내게 Look Scanned는 **마지막 전달 단계 도구**다.

문서를 올바르게 레닥션하고 최종 내보내기까지 검증한 다음, Look Scanned로 깔끔한 **스캔 느낌의 PDF**를 만든다. 공식 제출이나 포멀한 교환에서 사람들이 실제로 기대하는 형태다.

실무적으로는 다음 이점이 있다.

- “내 컴퓨터에서 서식이 바뀌었다”는 대화가 줄어든다  
- 더 강한 완성본 느낌을 준다(특히 스캔본 기대가 있을 때)  
- 불필요한 마크업 레이어가 섞일 가능성을 낮추기 쉽다(내보내기 경로에 따라 다름)
