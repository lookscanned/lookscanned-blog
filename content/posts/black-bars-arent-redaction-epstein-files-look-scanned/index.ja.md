...
# 黒塗りバーはレダクションではない（そう、“Epstein files”のPDF騒動がまた思い出させた）
...
つまり、画面上では隠せていても、下層テキストやOCR、残留オブジェクトをそのまま送ってしまうことがあります。これが **無効なレダクション（ineffective redaction）** です。高度な攻撃ではなく、「覆う」と「削除する」を取り違えたワークフローの問題です。
...
確認対象は **最終書き出しファイル** のみ（実際に upload/email/share するもの）です。

- 機微語句を **Search**（氏名、ID、メール断片、住所）
- 黒塗り周辺を選択して **copy/paste** し、プレーンテキストで確認
- **2種類のviewer** で開く（デスクトップアプリ + ブラウザで十分なことが多い）
- 残った **annotations/comments** を確認（highlights, notes, shapes）
- 外部送付なら **metadata**（author/title/subject）も確認

文書がスキャン起点だったりOCRを通している場合は、特に注意します。非表示なのに検索できるテキストは典型的な “surprise layer” だからです。
...
1) **本物のレダクションを行う**（overlayではなくコンテンツ削除）  
2) **余計なレイヤーを掃除する**（annotations, attachments, hidden layers, metadata）  
3) **最終書き出しを検証する**（上のチェックリスト）  
4) **送付用バージョンを作る**（多くはscan-styleで、見た目を最終化）

最後の工程は思っている以上に重要です。いわゆる “security theater” のためではなく、端末差による崩れや偶発的なノイズを減らすためです。

## 私の中でのLook Scannedの役割

Look Scannedはレダクションツールとして使っていません。役割が違います。  
私にとっては **last-mile deliverable tool** です。

文書を正しくレダクションし、最終ファイルを検証したあとに、Look Scannedでクリーンな **scan-style PDF** を作ります。提出書類やフォーマルなやり取りで期待される、あの仕上がりです。

実務上のメリットは次の通りです。

- 「私の環境だとレイアウトが崩れる」という往復が減る  
- 「最終成果物らしさ」が出る（特にスキャン見た目が求められる場合）  
- 余計なマークアップ層の混入リスクを下げやすい（export pipeline次第）

順序が重要です。**remove → verify → finalize**。
